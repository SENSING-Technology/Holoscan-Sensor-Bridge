...
class HoloscanApplication(holoscan.core.Application):
    ...
    def compose(self):
        ...
        if self._print_time:
            ...
            self.add_flow(timestamp_printers[0], uyvy_converter_left, {("output", "input")})
            self.add_flow(receiver_operator_right, timestamp_printers[1], {("output", "input")})
            self.add_flow(timestamp_printers[1], uyvy_converter_right, {("output", "input")})
        else:
