...
from . import li_i2c_expander,  sg3_isx031_gmsl2f_mode

# Camera info
DRIVER_NAME = "SG3_ISX031_GMSL2F"
VERSION = 1.0

class Isx031fCam(hololink_module.Synchronizable):
    def __init__(
        self,
        hololink_channel,
        ...
        self._mode =  sg3_isx031_gmsl2f_mode.Sensor_Mode.Unknown
        ...
        if self._position == "front" or self._position == "left":
            self._sensor_addr =  sg3_isx031_gmsl2f_mode.SENSOR_0_I2C_ADDRESS
            self._ser_addr =  sg3_isx031_gmsl2f_mode.SER_0_I2C_ADDRESS
        elif self._position == "back" or self._position == "right":
            self._sensor_addr =  sg3_isx031_gmsl2f_mode.SENSOR_1_I2C_ADDRESS
            self._ser_addr =  sg3_isx031_gmsl2f_mode.SER_1_I2C_ADDRESS
        self._des_addr =  sg3_isx031_gmsl2f_mode.DSER_I2C_ADDRESS
    ...
    def start(self):
        ...
        for i2c_addr, reg, val in  sg3_isx031_gmsl2f_mode.sensor_start:
            if i2c_addr ==  sg3_isx031_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                time.sleep(val / 1000)  # the val is in ms
            elif i2c_addr ==  sg3_isx031_gmsl2f_mode.SER_DEF_I2C_ADDRESS:
                self.set_register(self._ser_addr, reg, val)
            else:
                if reg == 0x301A and (self._vsync.is_enabled()):
                    val = 0x29
                self.set_register(self._sensor_addr, reg, val)

    def stop(self):
        """Stop Streaming"""
        self._vsync.detach(self)
        for i2c_addr, reg, val in  sg3_isx031_gmsl2f_mode.sensor_stop:
            if i2c_addr ==  sg3_isx031_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                time.sleep(val / 1000)  # the val is in ms
            elif i2c_addr ==  sg3_isx031_gmsl2f_mode.SER_DEF_I2C_ADDRESS:
                self.set_register(self._ser_addr, reg, val)
    ...
    def configure_camera(self, sensor_mode_set):
        ...
        serdes_list = []
        sensor_mode_list = []
        if sensor_mode_set.value ==  sg3_isx031_gmsl2f_mode.Sensor_Mode.sensor_mode_1920x1536_30fps_yuv.value:
            if self._position == "front" or self._position == "left":
                serdes_list =  sg3_isx031_gmsl2f_mode.serdes_config_front
            elif self._position == "back" or self._position == "right":
                serdes_list =  sg3_isx031_gmsl2f_mode.serdes_config_back
            else:
                logging.error(f"Unsupported Camera position: {self._position}")

            sensor_mode_list =  sg3_isx031_gmsl2f_mode.sensor_mode_1920x1536_30fps_yuv
        ...
        for i2c_addr, reg, val in serdes_list:
            if i2c_addr ==  sg3_isx031_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                time.sleep(val / 1000)  # the val is in ms
            elif i2c_addr ==  sg3_isx031_gmsl2f_mode.DSER_I2C_ADDRESS and reg == 0x0010:
                self._linkCfg = self.get_register(self._des_addr, 0x0010) & 0x03
                self._linkCfg |= val
                self.set_register(i2c_addr, reg, val) 
            else:
                self.set_register(i2c_addr, reg, val) 
        
        self.set_register(self._des_addr, 0x0010, self._linkCfg)
        time.sleep(0.2)
        ...
        if self._vsync.is_enabled():
            if self._position == "front" or self._position == "left":
                for i2c_addr, reg, val in  sg3_isx031_gmsl2f_mode.sensor_enable_sync_front:
                    if i2c_addr ==  sg3_isx031_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                        time.sleep(val / 1000)  # the val is in ms
                    else:
                        self.set_register(i2c_addr, reg, val)
            elif self._position == "back" or self._position == "right":
                for i2c_addr, reg, val in  sg3_isx031_gmsl2f_mode.sensor_enable_sync_back:
                    if i2c_addr ==  sg3_isx031_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                        time.sleep(val / 1000)  # the val is in ms
                    else:
                        self.set_register(i2c_addr, reg, val) 
    ...
    def set_exposure_reg_shs1(self, value=0x0C):
        ...
        time.sleep( sg3_isx031_gmsl2f_mode.SENSOR_WAIT_MS / 1000)

    def set_exposure_reg_shs2(self, value=0x0C):
        if value < 0x00:
            logging.warn(f"Exposure value {value} is lower than the minimum.")
            value = 0x00
    ...
    def set_mode(self, sensor_mode_set):
        if sensor_mode_set.value < len( sg3_isx031_gmsl2f_mode.Sensor_Mode):
            self._mode = sensor_mode_set
            mode =  sg3_isx031_gmsl2f_mode.sensor_frame_format[self._mode.value]
