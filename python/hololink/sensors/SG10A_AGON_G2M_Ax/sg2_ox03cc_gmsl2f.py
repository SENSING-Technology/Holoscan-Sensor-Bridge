...
from . import li_i2c_expander, sg2_ox03cc_gmsl2f_mode

# Camera info
DRIVER_NAME = "SG2_OX03CC_GMSL2F"
VERSION = 1.0

class Ox03cCam(hololink_module.Synchronizable):
    def __init__(
        self,
        hololink_channel,
        ...
        self._mode = sg2_ox03cc_gmsl2f_mode.Sensor_Mode.Unknown
        ...
        if self._position == "front" or self._position == "left":
            self._sensor_addr = sg2_ox03cc_gmsl2f_mode.SENSOR_0_I2C_ADDRESS
            self._ser_addr = sg2_ox03cc_gmsl2f_mode.SER_0_I2C_ADDRESS
        elif self._position == "back" or self._position == "right":
            self._sensor_addr = sg2_ox03cc_gmsl2f_mode.SENSOR_1_I2C_ADDRESS
            self._ser_addr = sg2_ox03cc_gmsl2f_mode.SER_1_I2C_ADDRESS
        self._des_addr = sg2_ox03cc_gmsl2f_mode.DSER_I2C_ADDRESS
    ...
    def start(self):
        ...
        for i2c_addr, reg, val in sg2_ox03cc_gmsl2f_mode.sensor_start:
            if i2c_addr == sg2_ox03cc_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                time.sleep(val / 1000)  # the val is in ms
            elif i2c_addr == sg2_ox03cc_gmsl2f_mode.SER_DEF_I2C_ADDRESS:
    ...
    def stop(self):
        ...
        for i2c_addr, reg, val in sg2_ox03cc_gmsl2f_mode.sensor_stop:
            if i2c_addr == sg2_ox03cc_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                time.sleep(val / 1000)  # the val is in ms
            elif i2c_addr == sg2_ox03cc_gmsl2f_mode.SER_DEF_I2C_ADDRESS:
                self.set_register(self._ser_addr, reg, val)
    ...
    def configure_camera(self, sensor_mode_set):
        ...
        serdes_list = []
        sensor_mode_list = []
        if sensor_mode_set.value == sg2_ox03cc_gmsl2f_mode.Sensor_Mode.sensor_mode_1920x1280_raw12_4lane_30fps_linear.value:
            if self._position == "front" or self._position == "left":
                serdes_list = sg2_ox03cc_gmsl2f_mode.serdes_config_front
            elif self._position == "back" or self._position == "right":
                serdes_list = sg2_ox03cc_gmsl2f_mode.serdes_config_back
            else:
                logging.error(f"Unsupported Camera position: {self._position}")

            sensor_mode_list = sg2_ox03cc_gmsl2f_mode.sensor_mode_1920x1280_raw12_4lane_30fps_linear
        ...
        for i2c_addr, reg, val in serdes_list:
            if i2c_addr == sg2_ox03cc_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                time.sleep(val / 1000)  # the val is in ms
            elif i2c_addr == sg2_ox03cc_gmsl2f_mode.DSER_I2C_ADDRESS and reg == 0x0010:
        ...
        if self._vsync.is_enabled():
            for i2c_addr, reg, val in sg2_ox03cc_gmsl2f_mode.sensor_enable_sync:
                if i2c_addr == sg2_ox03cc_gmsl2f_mode.SENSOR_TABLE_WAIT_MS:
                    time.sleep(val / 1000)  # the val is in ms
                else:
                    self.set_register(self._sensor_addr, reg, val)
    ...
    def set_exposure_reg_shs1(self, value=0x0C):
        ...
        time.sleep(sg2_ox03cc_gmsl2f_mode.SENSOR_WAIT_MS / 1000)
    ...
    def set_mode(self, sensor_mode_set):
        if sensor_mode_set.value < len(sg2_ox03cc_gmsl2f_mode.Sensor_Mode):
            self._mode = sensor_mode_set
            mode = sg2_ox03cc_gmsl2f_mode.sensor_frame_format[self._mode.value]
